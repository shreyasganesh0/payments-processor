...
## Decision
- correctness maintained by atleast-once semantics
    - db row is the source of truth and queue is the trigger allows for queue to
    retry as many times as it wants and only do jobs that havent been recorded as 
    completed
    - canTransition FSM + select.for(update) query allows for idempotency in updates
        ...
        - it is then blocked by the fsm logic post the transtion caused by one
    - per payment idempotency bank key
        - provided by the bank so even duplicates that reach the bank charges only once 

## Failure Windows
- if there is a crash b/n PROCESSING claim and finalize a procesing claim can be stranded
...
- per payment ordering not garunteed
